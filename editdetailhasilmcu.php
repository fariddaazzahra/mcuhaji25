...
// Cek jika pengguna sudah login DAN rolenya adalah dokter ATAU admin.
if (!isset($_SESSION['loggedin']) || !in_array($_SESSION['role'], ['dokter', 'admin'])) {
...
$stmt = $conn->prepare("SELECT p.nama_pasien, p.nik, p.jk, p.tgl_lahir FROM jadwalmcu j JOIN pasien p ON j.pasien_id = p.pasien_id WHERE j.jadwal_id = ?");
...
// Ambil data hasil MCU dan penunjang yang sudah ada
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $file_usg = $pempenunjang['hasil_usg'] ?? '';
        if (isset($_FILES['hasil_usg']) && $_FILES['hasil_usg']['error'] == 0) {
            $file_usg = time() . '_usg_' . basename($_FILES['hasil_usg']['name']);
            move_uploaded_file($_FILES['hasil_usg']['tmp_name'], $upload_dir . $file_usg);
        }
        $file_rongten = $pempenunjang['hasil_rongten'] ?? '';
        if (isset($_FILES['hasil_rongten']) && $_FILES['hasil_rongten']['error'] == 0) {
            $file_rongten = time() . '_rontgen_' . basename($_FILES['hasil_rongten']['name']);
            move_uploaded_file($_FILES['hasil_rongten']['tmp_name'], $upload_dir . $file_rongten);
        }

        $stmt_hasil = $conn->prepare("UPDATE hasilmcu SET td=?, tb=?, bb=?, kolesterol=?, asam_urat=?, catatan_dokter=?, status_kelayakan=? WHERE jadwal_id=?");
        $stmt_hasil->bind_param("sssssssi", $td, $tb, $bb, $kolesterol, $asam_urat, $catatan_dokter, $status_kelayakan, $jadwal_id);
        $stmt_hasil->execute();
        $stmt_hasil->close();
        
        $stmt_penunjang = $conn->prepare("UPDATE pempenunjang SET tes_urin=?, tes_hcv=?, tes_hiv=?, tes_bta=?, tes_hav=?, tes_hbv=?, usg=?, hasil_usg=?, rongten=?, hasil_rongten=? WHERE jadwal_id=?");
        $stmt_penunjang->bind_param("ssssssssssi", $tes_urin, $tes_hcv, $tes_hiv, $tes_bta, $tes_hav, $tes_hbv, $usg, $file_usg, $rongten, $file_rongten, $jadwal_id);
        $stmt_penunjang->execute();
        $stmt_penunjang->close();

        $conn->commit();
        header("Location: $redirect_page?update=success");
        exit();
...
<head>
    ...
    <title>Edit Hasil MCU - <?= htmlspecialchars($data['nama_pasien']) ?></title>
...
<body>
    ...
    <aside class="sidebar">
        ...
        <div class="user-info">
            <div class="avatar"><?= $_SESSION['role'] == 'admin' ? '👑' : '🩺' ?></div>
    ...
    <main class="main-content">
        <div class="header">
            <h1>Edit Hasil Pemeriksaan MCU</h1>
            <p class="subtitle">Perbarui hasil pemeriksaan untuk pasien.</p>
        </div>
        ...
        <form method="POST" enctype="multipart/form-data">
            ...
            <div class="form-card">
                <h3>🔬 Skrining Awal</h3>
                <div class="form-grid">
                    <div class="form-group"><label for="td">Tekanan Darah (mmHg)</label><input type="text" id="td" name="td" class="form-input" value="<?= htmlspecialchars($hasil['td'] ?? '') ?>" placeholder="cth: 120/80"></div>
                    <div class="form-group"><label for="tb">Tinggi Badan (cm)</label><input type="number" id="tb" step="0.1" name="tb" class="form-input" value="<?= htmlspecialchars($hasil['tb'] ?? '') ?>" placeholder="cth: 170.5"></div>
                    <div class="form-group"><label for="bb">Berat Badan (kg)</label><input type="number" id="bb" step="0.1" name="bb" class="form-input" value="<?= htmlspecialchars($hasil['bb'] ?? '') ?>" placeholder="cth: 65.5"></div>
            ...
            <div class="form-card">
                ...
                <div class="form-grid">
                    <div class="form-group"><label for="tes_urin">Tes Urin</label><select id="tes_urin" name="tes_urin" class="form-input"><option value="">-- Pilih --</option><option value="negatif" <?= ($pempenunjang['tes_urin'] ?? '')=='negatif'?'selected':'' ?>>Negatif</option><option value="positif" <?= ($pempenunjang['tes_urin'] ?? '')=='positif'?'selected':'' ?>>Positif</option></select></div>
                    <div class="form-group"><label for="tes_hcv">Tes HCV</label><select id="tes_hcv" name="tes_hcv" class="form-input"><option value="">-- Pilih --</option><option value="reaktif" <?= ($pempenunjang['tes_hcv'] ?? '')=='reaktif'?'selected':'' ?>>Reaktif</option><option value="non-reaktif" <?= ($pempenunjang['tes_hcv'] ?? '')=='non-reaktif'?'selected':'' ?>>Non-Reaktif</option></select></div>
                    <div class="form-group"><label for="tes_hiv">Tes HIV</label><select id="tes_hiv" name="tes_hiv" class="form-input"><option value="">-- Pilih --</option><option value="negatif" <?= ($pempenunjang['tes_hiv'] ?? '')=='negatif'?'selected':'' ?>>Negatif</option><option value="positif" <?= ($pempenunjang['tes_hiv'] ?? '')=='positif'?'selected':'' ?>>Positif</option></select></div>
                    <div class="form-group"><label for="tes_bta">Tes BTA</label><select id="tes_bta" name="tes_bta" class="form-input"><option value="">-- Pilih --</option><option value="negatif" <?= ($pempenunjang['tes_bta'] ?? '')=='negatif'?'selected':'' ?>>Negatif</option><option value="positif" <?= ($pempenunjang['tes_bta'] ?? '')=='positif'?'selected':'' ?>>Positif</option></select></div>
                    <div class="form-group"><label for="tes_hav">Tes HAV</label><select id="tes_hav" name="tes_hav" class="form-input"><option value="">-- Pilih --</option><option value="negatif" <?= ($pempenunjang['tes_hav'] ?? '')=='negatif'?'selected':'' ?>>Negatif</option><option value="positif" <?= ($pempenunjang['tes_hav'] ?? '')=='positif'?'selected':'' ?>>Positif</option></select></div>
                    <div class="form-group"><label for="tes_hbv">Tes HBV</label><select id="tes_hbv" name="tes_hbv" class="form-input"><option value="">-- Pilih --</option><option value="negatif" <?= ($pempenunjang['tes_hbv'] ?? '')=='negatif'?'selected':'' ?>>Negatif</option><option value="positif" <?= ($pempenunjang['tes_hbv'] ?? '')=='positif'?'selected':'' ?>>Positif</option></select></div>
                    ...
                    <div class="form-group"><label for="hasil_usg">Upload Ulang File USG</label><input type="file" id="hasil_usg" name="hasil_usg" class="form-input" accept="image/*,application/pdf"> <?php if (!empty($pempenunjang['hasil_usg'])): ?><a href="../uploads/<?= htmlspecialchars($pempenunjang['hasil_usg']) ?>" target="_blank">Lihat file saat ini</a><?php endif; ?></div>
            ...
            <div class="form-card">
                ...
                <div class="form-grid">
                    <div class="form-group" style="grid-column: 1 / -1;"><label for="catatan_dokter">Catatan</label><textarea id="catatan_dokter" name="catatan_dokter" rows="5" class="form-input" placeholder="Tuliskan kesimpulan, diagnosis, atau rekomendasi di sini..."><?= htmlspecialchars($hasil['catatan_dokter'] ?? '') ?></textarea></div>
            ...
            <div class="form-actions">
                <a href="<?= $redirect_page ?>" class="btn btn-secondary">Batal</a>
