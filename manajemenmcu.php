...
if (isset($_GET['delete']) && $_GET['delete'] == 'success') {
    $notification = ['type' => 'success', 'text' => 'Data MCU berhasil dihapus.'];
}
if (isset($_GET['update']) && $_GET['update'] == 'success') {
    $notification = ['type' => 'success', 'text' => 'Data MCU berhasil diperbarui.'];
}
if (isset($_GET['error'])) {
    $notification = ['type' => 'error', 'text' => 'Gagal memproses data. Mungkin data terkait tidak ada.'];
}

// --- PERBAIKAN LOGIKA FILTER & PENCARIAN ---
$search = $_GET['search'] ?? '';
$filter_waktu = $_GET['filter_waktu'] ?? 'semua'; // Default ke 'semua'

// Menyiapkan klausa WHERE dinamis
$where_clauses = [];
$params = [];
$types = "";
...
// Filter Pencarian
if (!empty($search)) {
    $where_clauses[] = "CONCAT(p.pasien_id, p.nama_pasien, p.nik, j.status_mcu, h.status_kelayakan) LIKE ?";
    $params[] = "%$search%";
    $types .= "s";
}
...
switch ($filter_waktu) {
    case 'hari_ini':
        $where_clauses[] = "DATE(j.tanggal_mcu) = CURDATE()";
        break;
    case 'minggu_ini':
        $where_clauses[] = "j.tanggal_mcu >= DATE_SUB(CURDATE(), INTERVAL 7 DAY)";
        break;
    case 'bulan_ini':
        $where_clauses[] = "MONTH(j.tanggal_mcu) = MONTH(CURDATE()) AND YEAR(j.tanggal_mcu) = YEAR(CURDATE())";
        break;
}
...
$sql = "
    SELECT 
        j.jadwal_id, p.pasien_id, p.nama_pasien, p.nik, p.jk, j.tanggal_mcu, j.status_mcu, h.status_kelayakan
    FROM jadwalmcu j
...
<head>
    ...
    <title>Manajemen MCU - Admin</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .action-section { background: white; padding: 1.5rem; border-radius: 20px; box-shadow: 0 4px 20px rgba(0, 0, 0, 0.08); margin-bottom: 1rem; }
        ...
        .filter-btn { padding: 0.7rem 1.2rem; border: 2px solid var(--medium-green-2); border-radius: 10px; font-weight: 600; font-size: 0.9rem; cursor: pointer; transition: all 0.3s ease; text-decoration: none; background-color: transparent; color: var(--medium-green-2); }
        .filter-btn.active, .filter-btn:hover { background-color: var(--medium-green-2); color: var(--white); }
...
<body>
   ...
   <main class="main-content">
        <div class="header">
            <h1>Manajemen Jadwal & Hasil MCU</h1>
            <p class="subtitle">Lihat, edit, dan hapus semua data jadwal dan hasil pemeriksaan medis pasien.</p>
        </div>
        
        <div class="action-section">
            <form method="GET" class="search-form">
                <input type="hidden" name="filter_waktu" value="<?= htmlspecialchars($filter_waktu) ?>">
                <input type="text" name="search" class="search-input" placeholder="🔍 Cari ID, Nama, NIK, atau Status..." value="<?= htmlspecialchars($search) ?>">
                <button type="submit" class="btn btn-primary">Cari</button>
                <a href="manajemenmcu.php" class="btn btn-secondary">Reset</a>
            </form>
        </div>
        
        <div class="filter-bar">
            <a href="?filter_waktu=semua&search=<?= urlencode($search) ?>" class="filter-btn <?= $filter_waktu == 'semua' ? 'active' : '' ?>">Semua Waktu</a>
            <a href="?filter_waktu=hari_ini&search=<?= urlencode($search) ?>" class="filter-btn <?= $filter_waktu == 'hari_ini' ? 'active' : '' ?>">Hari Ini</a>
            <a href="?filter_waktu=minggu_ini&search=<?= urlencode($search) ?>" class="filter-btn <?= $filter_waktu == 'minggu_ini' ? 'active' : '' ?>">7 Hari Terakhir</a>
            <a href="?filter_waktu=bulan_ini&search=<?= urlencode($search) ?>" class="filter-btn <?= $filter_waktu == 'bulan_ini' ? 'active' : '' ?>">Bulan Ini</a>
        </div>
        ...
        <div class="table-section">
            <?php if ($result && $result->num_rows > 0): ?>
                <table>
                    <thead>
                        <tr>
                            <th>No</th><th>Nama Pasien</th><th>NIK</th><th>Tanggal MCU</th><th>Status MCU</th><th>Kelayakan</th><th>Aksi</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php $no = 1; while($r = $result->fetch_assoc()): ?>
                            <tr>
                                <td><?= $no++ ?></td>
                                <td><strong><?= htmlspecialchars($r['nama_pasien']) ?></strong><br><small style="color: #6B9071;">ID Pasien: <?= $r['pasien_id'] ?></small></td>
                                <td><?= htmlspecialchars($r['nik']) ?></td>
                                <td><?= date('d M Y, H:i', strtotime($r['tanggal_mcu'])) ?></td>
                                <td><span class="badge status-<?= strtolower($r['status_mcu']) ?>"><?= htmlspecialchars(ucfirst($r['status_mcu'])) ?></span></td>
                                <td>
                                    <?php if(!empty($r['status_kelayakan'])): ?>
                                        <span class="badge status-<?= str_replace(' ', '-', strtolower(str_replace('/',' ',$r['status_kelayakan']))) ?>"><?= htmlspecialchars($r['status_kelayakan']) ?></span>
                                    <?php else: ?>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        <a href="../dokter/detailhasilmcu.php?id=<?= $r['jadwal_id'] ?>" class="btn btn-primary btn-sm">👁️ Lihat</a>
                                        <a href="editmcu.php?id=<?= $r['jadwal_id'] ?>" class="btn btn-warning btn-sm">✏️ Edit</a>
                                        <a href="hapusmcu.php" onclick="confirmDelete('hapusmcu.php?id=<?= $r['jadwal_id'] ?>', '<?= htmlspecialchars(addslashes($r['nama_pasien'])) ?>')" class="btn btn-danger btn-sm">🗑️ Hapus</a>
                                    </div>
                                </td>
                            </tr>
                        <?php endwhile; ?>
            ...
            <?php else: ?>
                <div class="empty-state"><div class="icon">🗂️</div><p>Data tidak ditemukan untuk filter yang dipilih.</p></div>
            <?php endif; ?>
        </div>
    </main>
...
function confirmDelete(deleteUrl, patientName) {
    ...
    Swal.fire({
        title: 'Anda Yakin?',
        html: `Anda akan menghapus seluruh data MCU untuk <strong>${patientName}</strong>.<br>Tindakan ini tidak dapat dibatalkan!`,
